...
public interface DepartmentDao {
    ...
    //删除部门信息
    @Delete("delete from hr_department where name=#{name}")
    Integer delete_hr_department(Department dep);
    //更新部门信息
    @Update("update hr_department set name=#{name},manager=#{manager},updatetime=(now()),updateUser=#{updateUser}")
    ...
    //根据部门名称查询信息
//    @Select("select id,name,manager from hr_department where name=#{name}")
//    Department querybyname_hr_department();
}
